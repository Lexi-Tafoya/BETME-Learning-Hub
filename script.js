...
/* 2 ----------------------------------------------------------------------- */
{
  ...
  items: [
    'Why TMR exists',
    'How TMR evolved in Operations',
    'Career paths and accountability',
    ...
    'Leader preparation',
    'Evidence',
    'Calibration and the Operations offsite',
    'Actions after classification',
    'Business Enablement next steps'
  ],
  notes: {
    who: 'Lexi',
    point: 'Orient the room. Twelve topics, building from why to what next.',
    say: 'Twelve topics. The first half is why TMR exists and how it works. The second half is '
...
function build(s, i) {
  ...
  switch (s.kind) {
    ...
    case 'agenda':
      h = head(s) + `<ol class="agenda">${s.items.map((x, n) =>
            `<li><span class="ag-n">${String(n + 1).padStart(2, '0')}</span>
             <span class="ag-t">${x}</span></li>`).join('')}</ol>`;
      break;
